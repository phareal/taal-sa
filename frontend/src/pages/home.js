...
export async function renderHome(root) {
  ...
  const [kpiCurrent, kpiPrev, caAnnuel, caMensuel, topClients, compta] = await Promise.all([
    api.get(`/api/analytics/kpis?annee=${currentYear}`).catch(() => null),
    api.get(`/api/analytics/kpis?annee=${currentYear - 1}`).catch(() => null),
    api.get(`/api/analytics/ca-annuel`).catch(() => []),
    api.get(`/api/analytics/ca-mensuel?annees[]=${currentYear - 4}&annees[]=${currentYear - 3}&annees[]=${currentYear - 2}&annees[]=${currentYear - 1}&annees[]=${currentYear}`).catch(() => []),
    api.get(`/api/analytics/ca-par-client?limit=10`).catch(() => []),
    api.get(`/api/comptabilite/summary?annee=${currentYear}`).catch(() => null),
  ]);
  ...
  const heatmapYears = Object.keys(heatmapMap).map(Number).sort();
  ...
  const heatCellStyle = (moisNum, year) => {
    ...
    if (!v) return `padding:3px; text-align:center; border-radius:3px; font-size:9px; color:var(--text3);`;
    ...
    return `padding:3px; text-align:center; border-radius:3px; font-size:9px; font-weight:500;
            background:${bg}; color:${intensity > 0.5 ? "#0a0b0d" : "#f0ede8"};`;
  };
  ...
  root.innerHTML = `
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
      <div class="kpi-card">
        ...
        ${kpiCurrent && kpiPrev ? `<div class="kpi-delta ${deltaCls(kpiCurrent.ca_total, kpiPrev.ca_total)}">${delta(kpiCurrent.ca_total, kpiPrev.ca_total)} vs ${currentYear - 1}</div>` : ""}
      </div>
      <div class="kpi-card">
        <div class="kpi-label">Expéditions ${currentYear}</div>
        <div class="kpi-value">${kpiCurrent?.nb_bl ?? "—"} <span class="kpi-sub">B/L</span></div>
        ${kpiCurrent && kpiPrev ? `<div class="kpi-delta ${deltaCls(kpiCurrent.nb_bl, kpiPrev.nb_bl)}">${delta(kpiCurrent.nb_bl, kpiPrev.nb_bl)} vs ${currentYear - 1}</div>` : ""}
      </div>
      <div class="kpi-card">
        ...
        ${kpiCurrent && kpiPrev?.volume_m3 ? `<div class="kpi-delta ${deltaCls(kpiCurrent.volume_m3, kpiPrev.volume_m3)}">${delta(kpiCurrent.volume_m3, kpiPrev.volume_m3)} vs ${currentYear - 1}</div>` : ""}
      </div>
      <div class="kpi-card">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
      <div class="app-card lg:col-span-2">
        <div class="card-header"><div class="card-title">CA mensuel — heatmap</div><span class="card-badge">par année</span></div>
        <div style="overflow-x:auto;">
          ...
          <table style="width:100%; border-collapse:collapse; font-size:10px;">
            <thead>
              <tr>
                ...
                ${heatmapYears.map(y => `<th style="color:var(--text2); padding:3px; text-align:center; font-weight:500; min-width:50px;">${y}</th>`).join("")}
              </tr>
            </thead>
            <tbody>
          ...
          </table>` : `<div style="height:160px; display:flex; align-items:center; justify-content:center; color:var(--text3);">Aucune donnée mensuelle</div>`}
        </div>
      </div>
      <div class="app-card">
        <div class="card-header"><div class="card-title">Top 5 Clients</div><span class="card-badge">CA Total</span></div>
    ...
    <!-- Top clients table -->
    <div class="app-card">
      <div class="card-header"><div class="card-title">Top Consignataires — CA Total</div><span class="card-badge">Toutes années</span></div>
  ...
  return () => charts.forEach(c => { try { c.destroy(); } catch (_) {} });
}
